...
def create_parser():
    ...
    parser.add_argument("-skip_practice", "--skip_practice", help="Skip practice")
    parser.add_argument(
        "-single_block", "--single_block", help="Only complete a single block"
    )
...
def run(args=None):
    ...
    if args.skip_practice:
        web.config.update({"skip_practice": args.skip_practice})

    if args.single_block:
        web.config.update({"single_block": args.single_block})
...
def serve_experiment(experiment):
    ...
    if web.config.get("skip_practice", None):
        context["skip_practice"] = web.config.skip_practice
    if web.config.get("single_block", None):
        context["single_block"] = web.config.single_block
